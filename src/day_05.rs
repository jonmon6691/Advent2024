...
pub fn get_rules_and_updates(filename: &str) -> Result<(HashMap<u32, HashSet<u32>>, Vec<Vec<u32>>), String> {
...
pub fn do_d05_2() -> Result<u32, String> {
    ...
    Ok(updates.iter().filter(|update| {
        ...
        let mut f = f.clone();
        f.sort_by(|a, b| {
            let a_before_b = what_comes_after.get(a).unwrap_or(&HashSet::new()).contains(b);
            let b_before_a = what_comes_after.get(b).unwrap_or(&HashSet::new()).contains(a);
